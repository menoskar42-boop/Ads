...
async function plan(ctx, goal, recentContext = []) {
  ...
  const sys = [
    ...
    'To navigate INSIDE a site use navigate_site (it follows the site\'s own links), NOT search_web (external engines rarely index a JS site\'s deep pages). For "ادخل موقع X ودوّر على Y" / "افتح سيلندر وهات فحص عربية" use navigate_site with input.url = the site or its listing page and input.goal = what to find. Use browse only for a SINGLE known page.',
