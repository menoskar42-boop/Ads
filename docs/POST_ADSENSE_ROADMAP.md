...
**اتنفّذ ورُفِع + متحقّق على Postgres حقيقي (29 مرحلة):**
...
21 تحكّم الأدمن في المميزات (feature flags) · 22 تقارير مبيعات · **23 دليل استخدام /help** ·
24 Pixel+Feed · **26 سلة متروكة** · **27 طلب عبر واتساب** · 28 استيراد CSV ·
**29 تحليلات المتجر** · **31 بطاقات هدايا/محفظة**.
دورة الشراء الكاملة (deal→coupon→points→wallet→shipping→total + earning) مختبرة رقمياً، وكل ميزة ليها سويتش، وصفحة الدفع فيها ملخص وإجمالي.

**كمان اتنفّذ:** **30 صفحات هبوط لمنتج واحد** (noindex + canonical) · **32 اشتراكات متكررة** (تجديد يومي COD + كرون داخلي/خارجي، مختبر رقمياً) · **33 تعدّد العملات** (محوّل عرض).

**الباقي (2 — تكاملات خارجية تحتاج مفاتيح/حسابات فقط):**
- 14 بوّابات دفع per-tenant (تشفير مفاتيح + Paymob/Fawry — البنية الأساسية موجودة في `src/lib/gateways`) · جزئي: `payment_method` متسجّل + Paymob webhook.
- 25 تكامل شركات الشحن (Bosta/Aramex — يحتاج API keys من التاجر).

**الخلاصة: 31 من 33 مرحلة اتنفّذت. الباقي (14 و25) تكاملات خارجية بتحتاج مفاتيح/حسابات من التاجر مش كود.**
