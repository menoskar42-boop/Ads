...
> #### 🧠 مميزات ذكاء اصطناعي إضافية على صفحة المطعم (مدفوعة فقط) — مستوحاة من منافس "منيو مصر"
> **القاعدة الأساسية:** **المطعم والمنيو والطلب مجاني بالكامل للجميع.** ميزات الذكاء
> الاصطناعي دي **حصرية للنسخة المدفوعة** (نفس الـ gating: `food_ai_subscriptions.status='active'`).
> يعني التاجر ياخد مطعمه كامل مجاناً، ولو **عايز الذكاء الاصطناعي يدفع**.
>
> **⚠️ غير منفّذة بعد — توثيق/رودماب فقط (لحين موافقة AdSense وبدء بناء منصّة الطلبات).**
>
> **(8-ب) مساعد Upsell ذكي أثناء الطلب (مدفوع):**
> - أثناء ما العميل بيطلب، الـ AI يقترح **إضافات/مكمّلات تليق مع وجبته** (مثلاً: مشروب/جانبية/تحلية)
>   لزيادة قيمة الفاتورة (average order value).
> - الاقتراحات **مبنية على منيو التاجر نفسه فقط** (مش محتوى مخترع) — مقيّدة بالأصناف الموجودة.
> - يظهر بشكل لبق غير مزعج، والتاجر يقدر يفعّله/يوقفه.
>
> **(8-ج) تقارير وتحليلات ذكية للتاجر (مدفوع):**
> - تحليل حركة المبيعات: **الأكثر والأقل مبيعاً**، أوقات الذروة، متوسط الفاتورة.
> - **اقتراحات تعديل المنيو/الأسعار** مبنية على بيانات مبيعات التاجر الفعلية.
> - مقارنة أداء الأصناف، وتنبيهات (صنف راكد / صنف رائج) — قرارات مبنية على بيانات.
> - لوحة تقارير في حساب التاجر (تظهر فقط مع الاشتراك المدفوع).
>
> **gating موحّد:** نفس اشتراك الـ AI (المرحلة 8) بيفتح: مساعد الطلب + Upsell + التقارير الذكية.
> بدون اشتراك → المطعم شغّال كامل بـ COD ومنيو وطلبات، **بدون** أي ميزة AI.
>
> **التزام السياسات:** مخرجات الـ AI **مقيّدة ببيانات/منيو التاجر** فقط — لا محتوى مضلِّل
> (التزاماً بقواعد AdSense وSearch المخزّنة).
