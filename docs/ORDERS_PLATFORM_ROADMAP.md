...
### المرحلة 9 — الإعلانات (AdSense) على كل صفحات النوع الجديد
- إضافة slots في إعدادات الـads لكل سطح جديد (المنيو، السلة، تتبّع الطلب، لوحة العميل).
- متشيلش أي slot قائم؛ ضيف الجديد بس.
...
5. **AdSense:** سطح جديد = slot جديد في الـconfig؛ متشيلش حاجة قائمة.
