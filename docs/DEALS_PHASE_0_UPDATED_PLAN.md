...
أصبح لدينا الآن دليل مباشر من لوحة Associates الخاصة بالحساب يثبت أن واجهة Creators API ظاهرة للحساب. هذا لا يثبت بعد منح Product API access أو نجاح طلب API، ولا يثبت وحده توفر كل موارد Amazon.eg.
...
| ظهور بوابة Creators API داخل الحساب | CONFIRMED |
| اعتماد حساب Associates المطلوب للـ API | UNKNOWN / NEEDS VERIFICATION |
| أهلية Product API لحسابنا | UNKNOWN / NEEDS VERIFICATION |
| دعم Creators API لـ Amazon.eg في طلب فعلي | UNKNOWN / NEEDS VERIFICATION |
...
الصورة المرفقة من لوحة Amazon Associates تؤكد أن:

- لوحة Associates المصرية/العربية يمكن الوصول إليها بنجاح.
- الحساب يعرض قسمًا مستقلًا لمعلومات المدفوعات والضرائب.
- توجد داخل الحساب روابط لإدخال أو تعديل معلومات الحساب البنكي وبيانات الضرائب.
- حالة بيانات الضرائب/الدفع الظاهرة في اللوحة ما زالت غير مكتملة.
- الوصول إلى لوحة Associates لا يثبت قبول الحساب النهائي ولا يثبت توفر Creators API.

لم يتم حفظ صورة الشاشة أو Store ID أو أي معرّف حساب داخل هذا الملف.

### دليل Creators API من نص لوحة الحساب

النص الذي ظهر داخل لوحة الحساب يضيف المتطلبات الحالية التالية:

- يجب أن يكون لدى المستخدم حساب Associates/Creator معتمد.
- يجب تسجيل الدخول بحساب لديه صلاحية كاملة لإنشاء التطبيقات وتوليد Credentials.
- الوصول إلى Product API يتطلب **10 مبيعات مؤهلة على الأقل خلال آخر 30 يومًا**.
- بعد إنشاء Credentials قد تستغرق مراجعة الأهلية ومنح الوصول مدة تصل إلى 48 ساعة.
- قد يظهر الخطأ `AssociateNotEligible` أثناء فترة المراجعة.
- يمكن إنشاء تطبيقين كحد أقصى.
- لكل تطبيق مجموعتان من Credentials للتدوير، ويمكن استخدامهما بشكل مستقل.

هذه معلومات خاصة بواجهة الحساب الحالية، وهي أقوى من الاستنتاج من وثائق PA-API القديمة. لكنها لا تثبت أن الحساب الحالي اجتاز الشروط أو أن Credentials تم إنشاؤها بالفعل.
...
لا يوجد حتى الآن دليل من الحساب أو طلب API يثبت أن:
...
- الحساب الحالي لديه 10 مبيعات مؤهلة خلال آخر 30 يومًا.
- الحساب الحالي أصبح معتمدًا بالكامل.
- Product API access مُنح فعليًا.
- تم إنشاء Credentials صالحة.
- انتهت فترة المراجعة البالغة 48 ساعة.
- طلب API ناجح من Amazon.eg.

ويجب الآن تسجيل الفرق بين:

- شرط مراجعة Associates: 3 مبيعات مؤهلة خلال أول 180 يومًا.
- شرط Product API الظاهر داخل الحساب: 10 مبيعات مؤهلة خلال آخر 30 يومًا.
