...
function systemPrompt(list, lang, cur, merchantName, currentCart) {
  ...
  return [
    ...
    ``,
    `(معلومة داخلية ليك إنت بس — **ابدأ ردّك طبيعي زي «تمام، ضفتلك…» وممنوع تكتب كلمة «السلة الحالية» أو تسرد محتوى السلة للزبون** إلا لو هو سأل «إيه اللي في سلتي؟». اللي في السلة دلوقتي: ${cartAsText(currentCart).replace(/\n/g, ' | ')}. متضيفش صنف موجود فيها تاني؛ لو عايز تغيّر كميته استخدم update_cart.)`,
    ``,
    `المنيو (العملة: ${cur}):`,
