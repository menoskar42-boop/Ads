...
/**
 * بدائل اللغة لصفحة خليج — **الأسواق الشقيقة لنفس الموضوع**.
 *
 * ── الغلطة اللي الدالة دي اتكتبت بعدها ────────────────────────────────
 *
 * قراءة مانوس للـHTML المنشور (٢٠٢٦-٠٨-٢٦) لقت الأربع صفحات كلها بتقول
 * نفس الحاجة بالظبط: `hreflang="en"` بيشاور على نفسه، و`x-default`
 * بيشاور على نفسه. يعني صفحة السعودية وصفحة الإمارات — محتوى مختلف
 * لسوقين مختلفين — كل واحدة بتعلن إنها **هي** النسخة الإنجليزية الوحيدة
 * وإنها **هي** الافتراضي.
 *
 * ودي مش تفصيلة: مجموعة hreflang لازم تكون متبادلة، وكل عضو فيها بلغة
 * أو إقليم مختلف. صفحتين بنفس الوسم `en` مش مجموعة — دول مرشّحين
 * لنفس المكان، وجوجل بتختار واحدة وتسيب التانية. والـ`x-default`
 * المكرّر بيأكّد التعارض.
 *
 * الصح: `en-SA` و`en-AE` — كل صفحة بتسرد الاتنين (متبادل)، و**مفيش
 * `x-default`** لأن مافيش فيهم صفحة عامة بلا استهداف إقليمي. إعلان
 * افتراضي مش موجود هو نفس عيلة «hreflang بيكدب» اللي الملف ده متبني
 * على منعها.
 *
 * الوسم محسوب (`lang-CODE`) مش مكتوب، فسوق جديد في `markets.js` بيدخل
 * المجموعة لوحده.
 */
function alternatesFor(path) {
  ...
  if (!hit) return null;
  return pages()
    .filter((p) => p.topic === hit.topic)
    .map((p) => ({
      lang: `${MARKETS[p.market].lang}-${MARKETS[p.market].code}`,
      path: p.path,
    }));
}
...
module.exports = { TOPICS, LOCAL, pages, build, alternatesFor };
