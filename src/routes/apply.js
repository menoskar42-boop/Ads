...
router.get('/apply', (req, res) => {
  const preType = ['shop', 'portfolio', 'pharmacy', 'orders', 'clinic', 'gym', 'furniture'].includes(req.query.type) ? req.query.type : '';
...
router.post('/apply', async (req, res) => {
  ...
  if (!['shop', 'portfolio', 'pharmacy', 'orders', 'clinic', 'gym', 'furniture'].includes(values.business_type)) return render('اختر نوع الموقع.');
  ...
  try {
    ...
    const crmCategory = values.business_type === 'shop' ? 'متجر'
      ...
      : values.business_type === 'furniture' ? 'موبيليا/ورشة' : 'بورتفوليو';
    pool.query(
