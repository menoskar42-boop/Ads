...
router.get('/apply', (req, res) => {
  ...
  const preType = ['shop', 'portfolio', 'pharmacy', 'orders', 'clinic', 'gym', 'furniture', 'nutrition', 'workshop', 'hall', 'nursery'].includes(req.query.type) ? req.query.type : '';
...
router.post('/apply', applyLimiter, async (req, res) => {
  ...
  if (!['shop', 'portfolio', 'pharmacy', 'orders', 'clinic', 'gym', 'furniture', 'nutrition', 'workshop', 'hall', 'nursery'].includes(values.business_type)) return render('اختر نوع الموقع.');
  ...
  try {
    ...
    const crmCategory = values.business_type === 'shop' ? 'متجر'
      ...
      : values.business_type === 'nursery' ? 'حضانات/مراكز دروس' : 'بورتفوليو';
    pool.query(
