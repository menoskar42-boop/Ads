...
router.get('/:token', async (req, res) => {
  ...
  const [parts, labour, inspection, photos] = await Promise.all([
    pool.query('SELECT name, qty, unit_price FROM workshop_job_parts WHERE job_id=$1 ORDER BY id', [data.job_id]),
    pool.query('SELECT description, amount FROM workshop_job_labour WHERE job_id=$1 ORDER BY id', [data.job_id]),
    pool.query(
      `SELECT system, check_name, status, note, recommendation
         FROM workshop_inspection_items
        WHERE job_id=$1 AND customer_visible ORDER BY id`, [data.job_id]
    ),
    pool.query(
      `SELECT phase, image_url, caption FROM workshop_job_photos
        WHERE job_id=$1 ORDER BY id`, [data.job_id]
    ),
  ]);
  ...
  res.render('workshop_public/status', {
    ...
    inspection: inspection.rows, photos: photos.rows, totals,
...
router.post('/:token/approve', async (req, res) => {
  ...
  const [parts, labour] = await Promise.all([
    pool.query('SELECT qty, unit_price FROM workshop_job_parts WHERE job_id=$1', [data.job_id]),
    pool.query('SELECT amount FROM workshop_job_labour WHERE job_id=$1', [data.job_id]),
  ]);
  const totals = J.jobTotals(data, parts.rows, labour.rows);
