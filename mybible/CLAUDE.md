...
**Exception — tafsir (commentaries) are NOT embedded.** They are CSV files read at
runtime. There are two directories and only one of them is in git:

| Directory | In git? | Holds |
|---|---|---|
| `client/public/tafsir/` | ❌ gitignored | full-book CSVs — absent from every fresh clone |
| `client/public/tafsir-parts/` | ✅ tracked | 73 files, ~35 MB — **this is what actually serves** |

A fresh clone therefore has **73 books / 1308 of 1326 chapters, 0 books missing**
— not "مزامير only". `listAvailableBooks()` and `loadEntries()` read both
directories and prefer the part file, so nothing depends on the gitignored one.
Verify with `node scripts/check-tafsir-offline.js` from the parent repo root. It
runs the real service inside a sandbox containing **only git-tracked files**, and
fails if coverage drops or a part file is untracked.

The 18 genuinely missing chapters are: عدد 19 · حزقيال 3 · حكمة سليمان 4,8 ·
يشوع بن سيراخ 20,49 · باروخ 5 · المكابيين الأول 2-7,10,12-14,16. Two of them
(عدد 19, حزقيال 3) are fetched live from St-Takla on demand.

The line "المصدر: تفسير عربي منشور على St-Takla.org" under every commentary is a
**static author credit** in `TafsirText.tsx`, not a sign of a live fetch.

Do NOT wire up `client/src/lib/tafsir-data.ts` or `client/public/tafsir-links.md` —
their St-Takla URLs are generated by string concatenation, not verified, and a large
share of them 404. They are currently imported by nobody, which is correct.
