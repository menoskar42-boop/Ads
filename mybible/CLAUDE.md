...
runtime from `client/public/tafsir/`, which is **gitignored**. A fresh clone has
commentary for مزامير only (150/1189 chapters); every other book renders
"لا يوجد تفسير". Read `docs/TAFSIR_DATA.md` before touching anything tafsir-related,
and run `npx tsx script/audit-tafsir.ts` (on the machine that has the data) to see
exactly which books/chapters are missing.
