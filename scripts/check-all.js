...
const CHECKS = [
  ...
  ['check-form-fields-reach-db','كل حقل الراوت بيقراه من الفورم له خانة فعلاً'],
  ...
  ['check-crm-scale','لوحة CRM مابتسحبش كل الجدول في كل زيارة'],
  ['check-gulf','صفحات الخليج: إنجليزي حقيقي وفروق حقيقية وبلا ادعاء توافق'],
  ['check-demo-login','حساب الديمو مايدخلش بكلمة سر — من أي باب'],
  ['check-indexnow','تبليغ IndexNow تلقائي وبعناوين نهائية مش بتتحوّل'],
  ['check-manus-prompt','برومبت مانوس للسيو متطابق مع الكود (روابط وحقائق السايت‌ماب)'],
  ['check-sokro-prompt','برومبت اختبار سكرو متطابق مع الكود (مسارات ونطاقات حسّاسة)'],
  ['check-sokro-seo','سيو سكرو تحت نفس قاعدة الموقع (العنوان والوصف والإعلانات)'],
  ['check-sokro-auth','مفتاح توقيع سكرو مش نص معروف، والكوكي Secure'],
  ['check-sokro-contacts','جهات الاتصال مشفّرة، والاسم الغامض بيوقف مش بيتخمّن'],
  ['check-sokro-ring','سوكرو بيرنّلك: الرن بيوصل، والنداء ليه عمر، والمكالمة بتبدأ بكلام'],
  ['check-sokro-phone','المكالمة الهاتفية: صوت ثنائي الاتجاه، transcript، وتأكيد مملوك'],
  ['check-mybible-deutero','الأسفار القانونية الثانية: مفيش نص مقدّس في الكود، والاستيراد بيصلّح'],
  ['check-mybible-lectionary','القطمارس مايقولش «قراءة اليوم» على قراءات يوم تاني'],
  ['check-katameros-readings','قراءات القداس: المزمور والإنجيل من قسم «الإنجيل» مش السنكسار'],
  ['check-tafsir-offline','التفاسير شغّالة من ملفات جيت لوحدها — والسحب من سانت تكلا آخر حل'],
  ['check-mybible-seed-gate','راوتات seed/fix في mybible ورا مفتاح — مش مفتوحة للكل'],
  ['check-mybible-qa-prompt','برومبت اختبار mybible أرقامه وخطوطه الحمرا مطابقة للكود'],
  ['check-guest-merge-phone','ضيف مايقدرش ياخد حساب عضو تاني من غير تليفون مطابق'],
  ['check-group-load','صفحة المجموعة تستحمّل ٧٠٠ عضو يفتحوا مع بعض'],
  ['check-liturgy-occasion','مناسبة القداس وهيتينية الموسم مضبوطين كل يوم في السنة'],
  ['check-mybible-health-probe','/api/health بيقيس البطء بدل ما نخمّن سببه'],
  ['check-pool-budget','مجموع اتصالات أوسكار ديفز وmybible تحت سقف سوبابيز'],
  ['check-pool-warm','الاتصال بالقاعدة يفضل دافي — فتحه من جديد بيكلّف ٨٠٠ مللي'],
  ['check-cohost','التطبيقات المستضافة: كل واحد بقاعدته، وبيرجع لما يقع'],
  ['check-serviceflow-env','كل متغيّر بيئة في Service Flow مكتوب في دليل الاستضافة'],
  ['check-serviceflow-boot','فشل في شغل الإقلاع مايوقّعش Service Flow'],
  ['check-userscript-base','سكربتات التمبر منكى بتبعت للدومين الحالي مش القديم'],
  ['check-popup-blocked','نافذة ممنوعة تقول سببها مش بتستنى المهلة في صمت'],
  ['check-c360-signal','الباتش اللي في تاب واحد يخلص بآخر رقم مش بأول رقم'],
  ['check-daily-update-visible','التحديث كل نص ساعة يقول نتيجته مش بس إنه مُفعَّل'],
  ['check-poll-budget','تاب جهاز التنفيذ مايأكلش كوتة الـWorker بتاعة كل السبدومينات'],
  ...
  ['check-report-indexes','العمود اللي التقرير بيفلتر بيه متفهرس'],
  ['check-r2-storage','صور الصيانة مابتضيعش وهي رايحة R2'],
  ['check-r2-prompt','برومبت تجهيز R2 مطابق لأسرار الكود'],
  ['check-photo-delete','حذف صورة الصيانة للسوبر أدمن بس ومابيسيبش ملف يتيم'],
  ['check-noreal-measure','«قياس بدون Real»: تاريخ History وuploaded_at مايتلخبطوش'],
  ['check-sf-user-response','Service Flow مايبعتش الباسورد للمتصفح مع الدخول'],
  ...
  ['check-reminder-health','تذكيرات الورشة لها بريد احتياطي ومنع تكرار'],
  ['check-workshop-alert-email-http','عزل سجل بريد تنبيهات الورشة عبر HTTP والجلسات'],
  ...
  ['check-loop-length-report','سيرفس فلو: تقرير طول الخط قصاد السرعة والاسكور من نفس صف القياس'],
  ['check-measure-stall','سيرفس فلو: باتش القياس الطويل مابيتقطعش وهو شغّال'],
  ['check-tech-line-ownership','سيرفس فلو: الفنى بيقيس الخط اللى الشاشة بتقول إنه بتاعه'],
  ['check-sf-excel-copy','سيرفس فلو: نسخ مسلسل طويل لإكسيل مايبوظش (١٥ رقم)'],
  ['check-nutrition-plan-safety','باني الخطة: المتعارض مع الحساسية مابيتحفظش من غير سبب، ومفيش «اتحفظ» على فشل'],
  ['check-nutrition-qa-prompt','برومبت QA التغذية: كل رقم متوقَّع = المعادلة'],
  ['check-sector-home','كل قطاع: الديمو بيفتح نظامه، وصاحبه لاقي رابطه في القايمة واللوحة'],
  ['check-company-render-session','كل شاشة في لوحة الشركة بتاخد session وعنوان — مفيش 500 على أول زيارة'],
  ['check-push-card','كارت الإشعارات: الديمو بيقول «من حسابك»، وفشل المتصفح غير فشل السيرفر'],
  ['check-terms-termination','الشروط: الإنهاء لسبب محدّد وبإنذار، والاسترداد ونسخة البيانات مكتوبين'],
  ['check-tenant-shadow','حجز التغذية ولينك حجز الجيم بيوصلوا لصفحة التاجر مش لشاشة الدخول'],
  ['check-nutrition-portal-password','مريض التغذية بيغيّر كلمة السر بالحالية وعلى صفّه بس'],
  ['check-nutrition-export','تصدير عملاء التغذية إكسيل لصاحب العيادة بس ومقيّد بالعيادة'],
  ['check-nutrition-whatsapp','«ابعت على واتساب»: PDF على الجهاز، مفيش رفع ولا رقم متخمَّن'],
  ['check-food-catalog','القائمة الجاهزة للأطعمة متّسقة ومابتكتبش فوق شغل الأخصائي'],
