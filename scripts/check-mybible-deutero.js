...
/* ── ١٠) سانت تكلا: الشروط اللي هم كتبوها ─────────────────────────────
 *
 * النص ملك عام (يسوعية ١٨٧٧) ومش ملكهم، وشروطهم بتسمح بالنقل مع ذكر
 * المصدر والرابط، والنقل الجماعي «غير مُستحَب» عندهم. فالتنفيذ لازم
 * يفضل محترم لللي طلبوه — والفحوص دي بتمنع الانزلاق:
 *
 *   · مهلة بين الطلبات (مش تفريغ متوازي)
 *   · `User-Agent` بيقول إحنا مين ومعاه رابطنا — الإخفاء هو التعدّي
 *   · النطاق محصور في st-takla.org (مايبقاش بوّابة لأي موقع)
 *   · مافيش زحف — روابط محدّدة بس
 */
if (/stTaklaProbe|STTAKLA_UA/.test(src)) {
  check('فيه مهلة بين الطلبات', /POLITE_DELAY_MS/.test(src) && /setTimeout/.test(src),
    'تفريغ متوازي على سيرفر خدمة تانية — وهم كاتبين إن النقل الجماعي غير مستحب.');
  check('و`User-Agent` بيعرّف بينا ومعاه رابطنا',
    /STTAKLA_UA[\s\S]{0,200}mybible\.oscardevs\.com/.test(src),
    'الإخفاء وراء user-agent عام هو اللي بيخلّي الجلب تعدّي.');
  check('والنطاق محصور في st-takla.org',
    /st-takla\\\.org\$\/i\.test\(g\.url\.hostname\)/.test(src)
    || /st-takla/.test(src) && /hostname/.test(src),
    'من غير الحصر، الراوت بيبقى بوّابة جلب لأي موقع.');
  check('والاستكشاف بيجيب صفحة واحدة',
    /stTaklaProbe/.test(src) && !/for \([\s\S]{0,60}pages/.test(src),
    'الاستكشاف المفروض صفحة واحدة — الباقي بيتم بعد ما نعرف الشكل.');
}
