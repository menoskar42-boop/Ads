...
/* ── ٨. كيان واحد في السكيمة، وNAP واحد ──────────────────────────────── */
{
  ...
  /* ── والبرومبتات كمان — دي اللي بتنشر العنوان فعلاً ──────────────────
   *
   * شيلنا العنوان من القوالب وفضل عايش في `GEO_EXTENSION_PROMPT.md`
   * و`MANUS_SEO_GEO_PROMPT.md` — والاتنين دول **بيتبعتوا لوكيل خارجي
   * عشان يسجّل بيهم في أدلة الأعمال**. يعني الحذف من الموقع كان
   * بيوقّف عرض العنوان على صفحة واحدة، والبرومبت كان لسه بينشره في
   * عشرين دليل. المنع لازم يغطّي النص اللي بيخرج بره مش الكود بس. */
  const docLeaks = fs.readdirSync(path.join(ROOT, 'docs'))
    .filter((f) => f.endsWith('.md'))
    .map((f) => 'docs/' + f)
    .concat(['CLAUDE.md'])
    .filter((f) => /العادلي|فريال|71111/.test(raw(f)));
  check('ولا في أي ملف برومبت أو توثيق',
    docLeaks.length === 0, docLeaks.join(', ') || 'كل docs/*.md + CLAUDE.md',
    'ملفات البرومبتات بتتبعت لوكيل بيسجّل بيها في الأدلة — العنوان فيها '
    + 'بينتشر أوسع من الموقع نفسه.');
